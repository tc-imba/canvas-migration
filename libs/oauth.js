...
class CanvasOAuth {
    ...
    request(method, arr, data, access_token) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            async.retry({
                    ...
                    if (method === 'post') {
                        oauth2._request("POST", url, {}, data, access_token, cb );
                        //oauth2.post(url, access_token, null, data, null, cb)
                    } else {
                        oauth2._request("GET", url, {}, "", access_token, cb );
                        //oauth2.get(url, access_token, cb);
                    }
     ...
     * @returns {Promise}
     */
    async get(arr, access_token) {
        return await this.request('get', arr, null, access_token)
    }

    async post(arr, data, access_token) {
        return await this.request('post', arr, data, access_token);
    }
}
